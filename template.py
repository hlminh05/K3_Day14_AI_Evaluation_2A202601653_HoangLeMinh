...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        if relevant_seen == 0:
            return 0.0
        score = precision_sum / relevant_seen
        return max(0.0, min(1.0, score))
...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        positional_bias = (
            ...
            and all(response_averages[0] > score + 0.05 for score in response_averages[1:])
        )
        return {
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                ...
                else "Review the failure trace and add a targeted regression case"
            )
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        actions = {
            "hallucination": "Add claim-level grounding checks and require evidence before returning policy facts",
            "irrelevant": "Improve intent routing and add query-rewriting examples for Student Services questions",
            "incomplete": "Retrieve all required conditions and prompt the generator to include dates, exceptions, and next steps",
            "off_topic": "Strengthen scope classification and route unsupported intents to a concise scoped response",
            "refusal": "Calibrate safety guardrails with allowed Student Services examples to reduce false refusals",
        }
        ...
        for failure_type, _ in sorted(
            ...
            suggestions.append(
                actions.get(
                    normalized,
                    f"Review {failure_type} traces, identify the shared root cause, and add regression coverage",
                )
            )

        fallback_actions = [
            "Add the failed cases to a versioned regression dataset and block metric drops greater than 0.05",
            "Inspect retrieval traces separately from generated answers to localize failures before changing prompts",
            "Calibrate heuristic and LLM-judge thresholds against independently reviewed human labels",
        ]
